...
// ==========================================
const releaseNotes = [
  // ★ 追加：v2.4.0 NFC対応とハイブリッドシステムの導入
  {
    version: 'v2.4.0',
    date: '2026.06.15',
    title: 'NFCスマート消費と自動/手動ハイブリッド管理の導入',
    features: [
      'Android端末限定の革新的な機能「NFCスマート消費」を導入。市販のNFCシールにスマホをかざすだけで、専用の消費画面が立ち上がり一瞬で在庫を減らすことが可能に',
      '商品の仕入れ（新規登録）・編集画面に「自動消費ON/OFF」スイッチを追加。完全自動で減らす商品と、NFC等で手動管理する商品をパントリー内で自由に混在できるハイブリッドシステムを実現',
      'Web NFC APIを利用し、外部アプリを一切使わずにHabiTapの画面から直接NFCタグへ情報を書き込める「内蔵NFCライター機能」を実装',
      'NFC書き込み待機中に、通信を安全に中断できるキャンセル機能（AbortController）を搭載し、途中で手を止めてもシステムに負荷をかけない安心のUXを提供',
      '自動消費がOFF（NFC運用中）の商品には、編集画面に専用の「NFC手動管理（設定済み）」バッジが点灯する視覚的なエスコート機能を追加'
    ],
    fixes: [
      '深夜の自動在庫チェック処理（Cron）のロジックを改修し、自動消費がOFFに設定された商品を確実にスルーする（勝手に在庫を減らさない）完璧な防波堤を構築'
    ]
  },
  {
    version: 'v2.3.5',
    date: '2026.03.14',
    title: '通知システムの完全自動化と消費ペース管理のUX向上',
    features: [
      'パントリー（在庫一覧）画面にて、各商品の「消費ペース（例：10日で1個）」が一目でわかる専用バッジを追加表示',
      '商品の編集（Edit）画面に「消費ペース」の入力欄を追加。在庫数やペースを変更した際に、システムが「残り日数」を自動で再計算・保存するスマートな仕様へ刷新',
      '無料プランにおける複数アカウント作成による不正利用（通知の二重受け取り）をシステムレベルで遮断する防波堤を確立（1端末につき最新の1アカウントのみ通知可能な仕様）'
    ],
    fixes: [
      '深夜の自動在庫チェック機能（Cron）が、セキュリティシステム（Middleware）に阻まれて自動発動しない問題を完全に解消し、本番環境での自動運用を開通',
      '期限切れのプッシュ通知宛先（410 Goneエラー）に対するエラーハンドリングを強化し、通知システムの安定性を大幅に向上'
    ]
  },
  {
    version: 'v2.3.0',
